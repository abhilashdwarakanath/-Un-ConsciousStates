...
spikeCountsAwake = zeros(nValChans,length(tAwake));
spikeCountsLSPre = zeros(nValChans,length(tLSPre));
spikeCountsLSPost = zeros(nValChans,length(tLSPost));
...
for iChan = 1:nValChans
    ...
    spikeCountsAwake(iChan,:) = baks(spkTimesAwake{iChan},tAwake,params.baksScale,params.baksShape);
    clear idx;

    idx = find(valRasterLSPre(iChan,:)==1);
    spkTimesLSPre{iChan} = tLSPre(idx);
    spikeCountsLSPre(iChan,:) = baks(spkTimesLSPre{iChan},tLSPre,params.baksScale,params.baksShape);
    clear idx;

    idx = find(valRasterLSPost(iChan,:)==1);
    spkTimesLSPost{iChan} = tLSPost(idx);
    spikeCountsLSPost(iChan,:) = baks(spkTimesLSPost{iChan},tLSPost,params.baksScale,params.baksShape);
    clear idx;
...
stateSeqAwake = smoothOverChunks(stateSeqAwake-1,params.history,params.Fs);
stateSeqLSPre = smoothOverChunks(stateSeqLSPre-1,params.history,params.Fs);
stateSeqLSPost = smoothOverChunks(stateSeqLSPost-1,params.history,params.Fs);

% Get state durations
[dursDownStateAwake, dursUpStateAwake] = getBinaryChunkDurations(stateSeqAwake);
dursDownStateAwake = dursDownStateAwake./params.Fs; dursUpStateAwake = dursUpStateAwake./params.Fs;

[dursDownStateLSPre, dursUpStateLSPre] = getBinaryChunkDurations(stateSeqLSPre);
dursDownStateLSPre = dursDownStateLSPre./params.Fs; dursUpStateLSPre = dursUpStateLSPre./params.Fs;

[dursDownStateLSPost, dursUpStateLSPost] = getBinaryChunkDurations(stateSeqLSPost);
dursDownStateLSPost = dursDownStateLSPost./params.Fs; dursUpStateLSPost = dursUpStateLSPost./params.Fs;

%% Plot ACFs and get synchrony index

[acAwake,~,SIAwake] = sig_autocorr(frAwake,params.Fs,params.history*30);
[acLSPre,~,SILSPre] = sig_autocorr(frLSPre,params.Fs,params.history*30);
[acLSPost,lags,SILSPost] = sig_autocorr(frLSPost,params.Fs,params.history*30);
...
figure;
plot(tForFittingFreqs(2:end-1),cycleFreqsLSPre(2:end-1),'LineWidth',2)
hold on
plot(tForFittingFreqs(2:end-1),cycleFreqsLSPost(2:end-1),'LineWidth',2)
box off
...
subplot(1,2,1)
plot(tForFittingTime(2:end-1),upDursLSPre(2:end-1),'LineWidth',2)
hold on
plot(tForFittingTime(2:end-1),upDursLSPost(2:end-1),'LineWidth',2)
...
subplot(1,2,2)
plot(tForFittingTime(2:end-1),downDursLSPre(2:end-1),'LineWidth',2)
hold on
plot(tForFittingTime(2:end-1),downDursLSPost(2:end-1),'LineWidth',2)
xlabel('state duration [s]')
ylabel('normalised density []')
...
ylabel('Neuron ID')
yyaxis right
plot(tAwake, (alphaAwake(1,:)),'LineWidth',1.5)
hold on
plot(tAwake,stateSeqAwake,'-g','LineWidth',1.5)
ylabel('P(UP State)')
xlabel('time [s]')
...
ylabel('Neuron ID')
yyaxis right
plot(tLSPre, (alphaLSPre(1,:)),'LineWidth',1.5)
hold on
plot(tLSPre,stateSeqLSPre,'-g','LineWidth',1.5)
ylabel('P(UP State)')
xlabel('time [s]')
title('Resting State')
xlim([0 20])
...
ylabel('Neuron ID')
yyaxis right
plot(tLSPost, (alphaLSPost(1,:)),'LineWidth',1.5)
hold on
plot(tLSPost,stateSeqLSPost,'-g','LineWidth',1.5)
ylabel('P(UP State)')
xlabel('time [s]')
title('Resting State')
xlim([0 20])
