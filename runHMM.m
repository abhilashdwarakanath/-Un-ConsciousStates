...
if ~isempty(session) && isnumeric(session) && session >= 1 && session <= 4
    if session == 3
        userInput = inputdlg('Enter FAHA number (2,3,4):', 'Integer Input', [1 40]);
...
binsForFitting = 0:binSize:(0.5/params.history); % Max frequency corresponds to smallest possible state duration
...
for i = 1:length(figure_handles)
    ...
    file_name = sprintf('Fig_%d.png', i);
